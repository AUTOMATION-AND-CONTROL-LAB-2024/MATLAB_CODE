...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(manual_control_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(manual_control_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(manual_control_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(manual_control_M, 0);
  ...
  // Initialize model
  manual_control_initialize();

  // External Mode initialization
  errorCode = extmodeInit(manual_control_M->extModeInfo, &rtmGetTFinal
    (manual_control_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(manual_control_M, true);
    }
  }
