...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(controller_3dof_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      controller_3dof_M->Timing.taskTime0;
    controller_3dof_step();

    // Get model outputs here

    // Trigger External Mode event
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(controller_3dof_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(controller_3dof_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(controller_3dof_M, 0);
  ...
  // Initialize model
  controller_3dof_initialize();

  // External Mode initialization
  errorCode = extmodeInit(controller_3dof_M->extModeInfo, &rtmGetTFinal
    (controller_3dof_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(controller_3dof_M, true);
    }
  }
