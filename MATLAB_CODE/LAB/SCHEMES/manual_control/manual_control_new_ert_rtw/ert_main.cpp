...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(manual_control_new_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      manual_control_new_M->Timing.taskTime0;
    manual_control_new_step();

    // Get model outputs here

    // Trigger External Mode event
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(manual_control_new_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(manual_control_new_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(manual_control_new_M, 0);
  ...
  // Initialize model
  manual_control_new_initialize();

  // External Mode initialization
  errorCode = extmodeInit(manual_control_new_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(manual_control_new_M, true);
    }
  }
