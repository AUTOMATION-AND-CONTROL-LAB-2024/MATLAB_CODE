...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(pid_controller_pitch_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      pid_controller_pitch_M->Timing.taskTime0;
    pid_controller_pitch_step();

    // Get model outputs here

    // Trigger External Mode event
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(pid_controller_pitch_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(pid_controller_pitch_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  pid_controller_pitch_terminate();
  extmodeReset();
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(pid_controller_pitch_M, 0);
  ...
  // Initialize model
  pid_controller_pitch_initialize();

  // External Mode initialization
  errorCode = extmodeInit(pid_controller_pitch_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(pid_controller_pitch_M, true);
    }
  }
