...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(controller_1dof_VSC_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      controller_1dof_VSC_M->Timing.taskTime0;
    controller_1dof_VSC_step();

    // Get model outputs here

    // Trigger External Mode event
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(controller_1dof_VSC_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(controller_1dof_VSC_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(controller_1dof_VSC_M, 0);
  ...
  // Initialize model
  controller_1dof_VSC_initialize();

  // External Mode initialization
  errorCode = extmodeInit(controller_1dof_VSC_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(controller_1dof_VSC_M, true);
    }
  }
