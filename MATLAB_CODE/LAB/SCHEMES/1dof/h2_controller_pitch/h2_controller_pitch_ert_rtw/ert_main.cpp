...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(h2_controller_pitch_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      h2_controller_pitch_M->Timing.taskTime0;
    h2_controller_pitch_step();

    // Get model outputs here

    // Trigger External Mode event
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(h2_controller_pitch_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(h2_controller_pitch_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(h2_controller_pitch_M, 0);
  ...
  // Initialize model
  h2_controller_pitch_initialize();

  // External Mode initialization
  errorCode = extmodeInit(h2_controller_pitch_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(h2_controller_pitch_M, true);
    }
  }
