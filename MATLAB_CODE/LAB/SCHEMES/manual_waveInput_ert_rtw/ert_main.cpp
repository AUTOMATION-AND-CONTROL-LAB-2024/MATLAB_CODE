...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(manual_waveInput_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      manual_waveInput_M->Timing.taskTime0;
    manual_waveInput_step();

    // Get model outputs here

    // Trigger External Mode event
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(manual_waveInput_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(manual_waveInput_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(manual_waveInput_M, 0);
  ...
  // Initialize model
  manual_waveInput_initialize();

  // External Mode initialization
  errorCode = extmodeInit(manual_waveInput_M->extModeInfo, &rtmGetTFinal
    (manual_waveInput_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(manual_waveInput_M, true);
    }
  }
