...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(test_RPY_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(test_RPY_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(test_RPY_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(test_RPY_M, 0);
  ...
  errorCode = extmodeInit(test_RPY_M->extModeInfo, &rtmGetTFinal(test_RPY_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(test_RPY_M, true);
    }
  }
