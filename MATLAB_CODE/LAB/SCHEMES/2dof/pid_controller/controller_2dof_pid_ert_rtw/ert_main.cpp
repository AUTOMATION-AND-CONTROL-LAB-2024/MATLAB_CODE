...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(controller_2dof_pid_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    extmodeSimulationTime_T currentTime = (extmodeSimulationTime_T)
      controller_2dof_pid_M->Timing.taskTime0;
    controller_2dof_pid_step();

    // Get model outputs here

    // Trigger External Mode event
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(controller_2dof_pid_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(controller_2dof_pid_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(controller_2dof_pid_M, 0);
  ...
  // Initialize model
  controller_2dof_pid_initialize();

  // External Mode initialization
  errorCode = extmodeInit(controller_2dof_pid_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(controller_2dof_pid_M, true);
    }
  }
