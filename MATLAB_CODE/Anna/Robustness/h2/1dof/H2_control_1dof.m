...
% alpha=0;
% radius=0.8;
% LMIconstr=LMIconstr+[[(radius^2-alpha^2)*Y-alpha*(A*Y+Y*A'+B*L+L'*B'), A*Y+B*L;     % LMI for circle of radius rho and centered in -alpha
%                     (A*Y+B*L)', Y]>= 1e-2*eye(2*ntot)]+[Y>=1e-2*eye(ntot)];

f           =   trace(S)+2e-2*gammaL + 1e-2*alphaY;
